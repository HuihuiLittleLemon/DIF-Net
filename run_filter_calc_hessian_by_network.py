...
def train_light_transformer(light_transformer, coords_xy, triangles, pro_img1, pro_img2, optim, iter_i, writer, orth_pic_height_fun, optim_view_id, init_z, W_cur):
    W_pro = pro_img1.shape[0]
    coords_xy = coords_xy/W_cur
    # samples, init_z_samples = random_points_in_triangle(coords_xy[triangles], init_z[triangles], num_points=1)
    # coords_w_samples = np.vstack([coords_xy, samples.reshape(-1, 2)])
    # init_z_w_samples = np.concatenate((init_z, init_z_samples.reshape(-1)), axis=0)  
    coords_w_samples = coords_xy
    ...
    if isinstance(init_z, float):
        t_pixels_w_samples = (torch.sigmoid(t_pixels_w_samples.squeeze())+ init_z).unsqueeze(dim=-1)
    else:
        t_pixels_w_samples = (torch.sigmoid(t_pixels_w_samples.squeeze())+ torch.from_numpy(init_z).cuda()).unsqueeze(dim=-1)
    # t_pixels_w_samples = torch.sigmoid(t_pixels_w_samples)

    H1, det_H1_w_samples = get_gradients_and_H1(t_pixels_w_samples, coordinates)
    ...
    if iter_i < 200:
        ...
        scale = 0.4 + 0.4* np.clip((iter_i - 200) / 100, 0.0, 1.0)
    ...
    if iter_i < 300:
        ...
        # gauss_weight = 0.01+1.0 * np.clip((iter_i - 500) / 500, 0.0, 1.0)
        gauss_weight = 1e5 + 9e5* np.clip((iter_i - 300) / 500, 0.0, 1.0)

    x_cord = W_pro*coords_w_samples[:,0]
    y_cord = W_pro*coords_w_samples[:,1]

    for idx in range(W_pro):
        yy = np.where(np.around(y_cord) == idx) #y_cord和x_cord不是整数
        if yy[0].size == 0:
            continue
        if optim_view_id != 3:
            direct_error += orth_pic_height_fun(pro_img1[:,idx],W_pro*t_pixels_w_samples[yy].squeeze(),scale).sum()
        else:
            direct_error += orth_pic_height_fun(pro_img1[idx,:],W_pro*t_pixels_w_samples[yy].squeeze(),scale).sum()
    ...
    for idx in range(W_pro):
        ...
        if xx[0].size == 0:
            continue
        if optim_view_id == 1:
            direct_error += orth_pic_height_fun(pro_img2[:,idx],W_pro*t_pixels_w_samples[xx].squeeze(),scale).sum()
        else:
            direct_error += orth_pic_height_fun(pro_img2[idx,:],W_pro*t_pixels_w_samples[xx].squeeze(),scale).sum()
    ...
    print(
        'iter:{:3>d} total_loss = {:.7f}, direct_error = {:.7f},gauss_error = {:.12f}'
        .format(iter_i, loss, direct_error, gauss_error))
    
    if optim_view_id == 1:
        out_cords = W_cur*torch.cat([coordinates[0:coords_xy.shape[0],:],t_pixels],dim=-1)
    elif optim_view_id == 2:
        out_cords = W_cur*torch.cat([t_pixels,coordinates[0:coords_xy.shape[0],[1,0]]],dim=-1) 
    else:
        out_cords = W_cur*torch.cat([coordinates[0:coords_xy.shape[0],1:2],t_pixels,coordinates[0:coords_xy.shape[0],0:1]],dim=-1) 
    det_H1 = det_H1_w_samples[0:coords_xy.shape[0]]
    return  out_cords.detach().cpu().numpy(), direct_error, gauss_error, t_pixels.squeeze(), loss, det_H1.detach().cpu().numpy()
...
def random_points_in_triangle(triangles, init_z, num_points=3):
    ...
    init_z_1 = init_z[:, 1]
    init_z_2 = init_z[:, 2]
    init_z_samples = lambda1[...] * init_z_0[:, None] + \
                     lambda2[...] * init_z_1[:, None] + \
                     lambda3[...] * init_z_2[:, None]

    return samples, init_z_samples

def get_black_pixel_bounds(img):
    """获取黑色像素的最小和最大x, y坐标以及黑色像素的中心"""
    W = img.shape[0]
    black_pixels = np.where(img == 0)
    min_x, max_x = black_pixels[1].min()/W, black_pixels[1].max()/W
    min_y, max_y = black_pixels[0].min()/W, black_pixels[0].max()/W
...
def main():
    ...
    if model == 'optimize':
        ...
        img1_gray = cv2.imread(os.path.join(meta_params['data_root'], meta_params['experiment_name'] + '/imgs/aligned_view1_512.png'), cv2.IMREAD_GRAYSCALE)#xy
        img2_gray = cv2.imread(os.path.join(meta_params['data_root'], meta_params['experiment_name'] + '/imgs/aligned_view2_256.png'), cv2.IMREAD_GRAYSCALE)#xz
        img3_gray = cv2.imread(os.path.join(meta_params['data_root'], meta_params['experiment_name'] + '/imgs/aligned_view3_512.png'), cv2.IMREAD_GRAYSCALE)#yz
        ...
        W_cur = cur_img.shape[0]
        W_pro = pro_img1.shape[0]

        h = 0.5
        points, triangles = generate_regular_triangle_mesh(cur_img, h)
        ...
        # 设置当前要拟合的的投影图像编号
        init_search_times = 10
        min_x, max_x, min_y, max_y = get_black_pixel_bounds(pro_img1)
        if optim_view_id == 1 or optim_view_id == 2:
            ...
            # init_z = 0.5 - np.array(np.where( scansum_xy == scansum_xy.max())).mean()/W

            # 将 [0, 1] 区间分成 10 份, 取中间点
            init_zs = np.linspace(min_x, max_x, init_search_times+2)[1:init_search_times+1] - 0.5
        else:          
            ...
            # scansum_y = scanproimg2.sum(axis=0)
            # scansum_xy =  scansum_x + scansum_y 
            # init_z = np.array(np.where( scansum_xy == scansum_xy.max())).mean()/W - 0.5
            # 获取原始黑色像素的最小和最大x, y坐标

            # 将 [0, 1] 区间分成 10 份, 取中间点
            init_zs = np.linspace(min_y, max_y, init_search_times+2)[1:init_search_times+1] - 0.5
        # int_z_list = []
        # int_z_list += init_zs.tolist()
        # ratio = (max_x - min_x)/(max_y - min_y)
        # init_z_skew1 = (ratio*(points[:,1]/W_cur - min_y) + min_x) - 0.5
        # init_z_skew2 = (-(points[:,0]/W_cur - max_x)/ratio + min_y) - 0.5
        # int_z_list.append(init_z_skew1)
        # int_z_list.append(init_z_skew2)
        int_z_list = []
        int_z_list += init_zs.tolist()
        ratio = (max_x - min_x)/(max_y - min_y)
        init_z_skew1 = (ratio*(points[:,1]/W_cur - min_y) + min_x) - 0.5
        init_z_skew2 = (-ratio*(points[:,1]/W_cur - min_y) + max_x) - 0.5
        init_z_skew3 = ((points[:,0]/W_cur - min_x)/ratio + min_y) - 0.5
        init_z_skew4 = (-(points[:,0]/W_cur - min_x)/ratio + max_y) - 0.5
        int_z_list.append(init_z_skew1)
        int_z_list.append(init_z_skew2)
        int_z_list.append(init_z_skew3)
        int_z_list.append(init_z_skew4)
        ...
        iter_num_little = 50
        init_z_min = torch.zeros((points.shape[0])).cuda()
        loss_min = 1e8
        for idx in range(len(int_z_list)):
            break
            init_z = int_z_list[idx]
            print("Seaching init z total {:2d} turn, current: {:2d}.".format(len(int_z_list), idx+1))
            for iter_i in tqdm.tqdm(range(iter_num_little)):
                cords, direct_error, gauss_error, z, loss, _ = train_light_transformer(light_transformer, points, triangles,
                    pro_img1, pro_img2, optim, iter_i, writer, orth_pic_height_fun, optim_view_id, init_z, W_cur)
            if loss_min > loss:
                init_z_min = z.detach().cpu().numpy()
                loss_min = loss
            ...
            o3d.io.write_triangle_mesh(f"./images_data/{meta_params['experiment_name']}/Net_output/init_z/{idx}_loss_{loss:.6f}_derror_{direct_error:.6f}_gerror{gauss_error:.6f}.ply", combined_mesh)
            # 重置网络参数
            light_transformer.reset_parameters()
            # 重新初始化优化器
            optim = torch.optim.Adam(lr=0.0001, params=light_transformer.parameters())

        init_z_min = init_z_min - 0.5
        print("Seaching done.")

        iter_num=int(meta_params['iter_num'])
        for iter_i in tqdm.tqdm(range(iter_num+1)):
            cords, direct_error, gauss_error, _, _, det_H1 = train_light_transformer(light_transformer, points, triangles,
             pro_img1, pro_img2, optim, iter_i, writer, orth_pic_height_fun, optim_view_id, init_z_skew4, W_cur)
            # if iter_i % 100 == 0:
            ...
            if iter_i % 100 == 0:
                ...
                # 设置点云数据
                combined_mesh.vertices = o3d.utility.Vector3dVector(cords)
                combined_mesh.triangles = o3d.utility.Vector3iVector(triangles)
                combined_mesh.paint_uniform_color(np.array([1.0, 0.0, 0.0]))
                # 将点云保存为ply文件
                ...
                plt.scatter(cords[:, 2], W_cur-cords[:, 1], c=det_H1, cmap='YlGn', marker='o', s=5)
                # 添加颜色条
                plt.colorbar()
                # 保存图片到文件，而不显示
                plt.savefig(f"./images_data/{meta_params['experiment_name']}/Net_output/heatmap/mesh_{iter_i}_heatmap.png", dpi=300, bbox_inches='tight')
                ...
                # 计算 det_H1 的阈值
                max_det_H1 = np.max(det_H1)
                percents = [1, 5, 10]

                for idx, percent in enumerate(percents):
                    threshold = max_det_H1*percent/100
                    plt.scatter(cords[:, 2], W_cur - cords[:, 1], c='blue', marker='o', s=5)  # 默认点为蓝色
                    # 找到大于阈值的点
                    above_threshold = det_H1 > threshold
                    # 将大于阈值的点绘制为红色
                    plt.scatter(cords[above_threshold, 2], W_cur - cords[above_threshold, 1], c='red', marker='o', s=5)
                    
                    # 添加颜色条
                    plt.colorbar()
                    
                    # 保存图片到文件，而不显示
                    plt.savefig(f"./images_data/{meta_params['experiment_name']}/Net_output/feture_points/mesh_{iter_i}_heatmap_threshold_{percent}.png", dpi=300, bbox_inches='tight')
